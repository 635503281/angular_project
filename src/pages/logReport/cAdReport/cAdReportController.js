(function(app){
    ...
    app.controller("cAdReportCtrl",cAdReportCtrl);
    cAdReportCtrl.$inject=["$scope","$http","$translate","$rootScope","commonMethod","cpService","nodeService","publicService","$filter"];
    function cAdReportCtrl($scope,$http,$translate,$rootScope,commonMethod,cpService,nodeService,publicService,$filter){
        ...
        $scope.init=()=>{
            //tablist
            $scope.tabList=[
                {id:1,name:$rootScope.cAdReport.reportDetailName,isShow:$rootScope.cAdReport.reportDetailBtn},
                {id:2,name:$rootScope.cAdReport.reportTrendName,isShow:$rootScope.cAdReport.reportTrendBtn}
            ];
            //当前tab页
                ...
                case 1://报表明细
                    ...
                    $scope.table=layui.table.render({
                        ...
                        cols:[[
                            ...
                            {field:"domain",title:$translate.instant("domainName"),align:"left",minWidth:120,
                            templet:`<div>{{d.domain||"--"}}</div>`
                            },
        ...
        $scope.getPageData=function(){
            if($scope.currentTab==1){
                ...
                if($scope.searchOptions.time&&$scope.searchOptions.groupType!=null){
                    let url=commonMethod.getServerUrl()+"/rest/statistics/daas/cp/domain/cacheInfo";//明细
                    let obj={
                ...
                if($scope.searchOptions.time&&$scope.searchOptions.groupType!=null){
                    let url=commonMethod.getServerUrl()+"/rest/statistics/daas/cp/domain/trend/cacheInfo";//趋势
                    let obj={
                        groupType:$scope.searchOptions.groupType,
                        cp:$scope.searchOptions.cp,
                        domain:$scope.searchOptions.domain,
        ...
        //导出
        $scope.export=()=>{
            let url=commonMethod.getServerUrl()+"/rest/statistics/daas/cp/domain/cacheInfo/excel";
            let obj={
                groupType:$scope.searchOptions.groupType||"",
                cp:$scope.searchOptions.cp,
                domain:$scope.searchOptions.domain,
