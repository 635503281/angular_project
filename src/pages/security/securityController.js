...
(function(app){
    ...
    app.controller("securityCtrl",securityCtrl);
    securityCtrl.$inject=['$scope','$state','$rootScope','menuService'];
    function securityCtrl($scope,$state,$rootScope,menuService){
        
        //初始化
        $scope.init=function(){
            //默认二级菜单，刷新还是当前菜单
            let defaultSecondMenuUrl=menuService.getFSecondMenu("index.security");
