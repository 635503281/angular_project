(function(app){
    ...
    app.controller("sendConfigCtrl",sendConfigCtrl);
    sendConfigCtrl.$inject=["$scope","$http","$translate","$state","commonMethod","ngDialog"];
    function sendConfigCtrl($scope,$http,$translate,$state,commonMethod,ngDialog){
        //初始化
        $scope.init=()=>{
            //分页参数
            $scope.pagingOptions={
            ...
            $scope.table=layui.table.render({
                ...
                cols:[[
                    ...
                    {field:"errorMessage",title:$translate.instant("errorMessage"),align:"left",minWidth:150,width:"20%",
                     templet:`<div>{{d.sendRecord.errorMessage||"--"}}</div>` 
                    },
                    {field:"createTime",title:$translate.instant("createTime"),sort:true,align:"left",width:160,
                     templet:`<div>{{new Date(d.sendRecord.createTime).format("yyyy-MM-dd hh:mm:ss")}}</div>`
                    }
        ...
        $scope.getPageData=function(){
            let url=commonMethod.getServerUrl()+"/rest/getSendRecords";//发送配置
